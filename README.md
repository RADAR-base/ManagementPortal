...
| `MANAGEMENTPORTAL_AUTHSERVER_TOKENURL`                       | None                                                | Full URL of the external auth server’s token endpoint. When using Ory Hydra, set to the token URL (e.g. `http://hydra:4444/hydra/oauth2/token`; maps to `managementportal.authServer.tokenUrl` in application-prod.yml).                                                                                   |
...
- **External Ory identity and auth (Kratos + Hydra)**:
  ...
  - Configure the external auth server (typically Ory Hydra):
    - `MANAGEMENTPORTAL_AUTHSERVER_TOKENURL` — full token URL (e.g. `http://hydra:4444/hydra/oauth2/token`, reachable from ManagementPortal)
