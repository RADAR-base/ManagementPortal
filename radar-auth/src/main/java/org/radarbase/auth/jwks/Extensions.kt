...
fun RSAPublicKey.toAlgorithm(hashSize: RSAJsonWebKey.HashSize = RSAJsonWebKey.HashSize.RS256): Algorithm =
    when (hashSize) {
...
fun ECPublicKey.toAlgorithm(): Algorithm {
    val keySize =
        when (val orderLength = params.order.bitLength()) {
...
fun ECPublicKey.toAlgorithm(keySize: ECDSAJsonWebKey.Curve): Algorithm =
    when (keySize) {
