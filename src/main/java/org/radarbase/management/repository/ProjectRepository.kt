...
interface ProjectRepository :
    ...
    RevisionRepository<Project, Long?, Int> {
    @Query(
        value =
    ...
    @Query(
        ...
        countQuery =
            "select distinct count(project) from Project project " +
                "WHERE project.projectName in (:projectNames) " +
                "OR project.organizationName in (:organizationNames)",
    )
    ...
    fun findOneWithEagerRelationships(
        @Param("id") id: Long?,
    ): Project?
    ...
    fun findOneWithEagerRelationshipsByName(
