...
public class ClaimsTokenEnhancer implements TokenEnhancer, InitializingBean {
    ...
    public OAuth2AccessToken enhance(OAuth2AccessToken accessToken,
        ...
        if (authentication.getPrincipal() instanceof Principal
            ...
            Optional.ofNullable(userRepository.findOneByLogin(userName)).ifPresent(user -> {
                var roles = user.roles.stream().map(role -> {
                    var auth = role.authority.name;
                    return switch (role.getRole().getScope()) {
                        case GLOBAL -> auth;
                        case ORGANIZATION -> role.organization.name + ":" + auth;
                        case PROJECT -> role.project.getProjectName() + ":" + auth;
                    };
                }).toList();
                ...
                Set<String> newScopes = currentScopes.stream().filter(scope -> {
                    Permission permission = Permission.ofScope(scope);
                    var roleAuthorities = user.roles.stream().map(Role::getRole)
                            .collect(Collectors.toCollection(() ->
                                    EnumSet.noneOf(RoleAuthority.class)));
                    return authorizationOracle.mayBeGranted(roleAuthorities, permission);
            ...
            List<String> sourceIds = assignedSources.stream().map(s ->
                    s.sourceId.toString()).toList();
        ...
        additionalInfo.put(GRANT_TYPE_CLAIM, authentication.getOAuth2Request().getGrantType());
        ...
        auditEventRepository.add(new AuditEvent(userName, GRANT_TOKEN_EVENT, auditData));
