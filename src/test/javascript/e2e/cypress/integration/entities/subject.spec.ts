...
describe('Subject e2e test', () => {
    ...
    it('should be able to create new subject', () => {
        cy.get('jhi-subjects button.create-subject').click();
        // cy.wait(1000);
        cy.get('[name=externalLink]').type('https://radar-base-test.org');
        cy.get('[name=externalId]').type('test-subject-1');
        cy.get('[name=personName]').type('Test Subject 1');
        cy.get('[name=dateOfBirth]').type('1980-01-01');
        cy.get('jhi-dictionary-mapper select').first().select('Human-readable-identifier');
        cy.get('jhi-dictionary-mapper input').first().type('Test Subject 1');
        cy.contains('jhi-dictionary-mapper button', 'Add').click()
    ...
    it('should be able to edit a subject', () => {
        ...
        // cy.wait(1000);
        cy.get('[name=externalLink]').clear().type('https://radar-base-test-edited.org');
        cy.contains('jhi-subject-dialog button.btn-primary', 'Save').click();
        cy.get('jhi-subjects .subject-row').should('have.length', 4);
    });
    ...
    it('should have load subject row with subject-id, external-id, status, project, sources and attributes columns', () => {
        ...
        cy.get('jhi-subjects .subject-row').first().find('.subject-row__content .subject-row__subject-id a')
    ...
    xit('should be able to filter subjects by human readable id', () => {
    ...
    it('should be able to filter subjects by date of birth from', () => {
        cy.get('#advanced-filter').click();
        cy.get('#field_date_of_birth_from').type('1979-01-01');
        cy.get('#apply-filter').click();
        cy.get('jhi-subjects .subject-row').should('have.length', 1);
        cy.get('#field_date_of_birth_from').type('1981-01-01');
        cy.get('#apply-filter').click();
        cy.get('jhi-subjects .subject-row').should('have.length', 0);
        cy.get('#field_date_of_birth_from').clear();
        cy.get('#apply-filter').click();
        cy.get('jhi-subjects .subject-row').should('have.length', 4);
        cy.get('#advanced-filter').click();
    });

    it('should be able to filter subjects by date of birth to', () => {
        cy.get('#advanced-filter').click();
        cy.get('#field_date_of_birth_to').type('1981-01-01');
        cy.get('#apply-filter').click();
        cy.get('jhi-subjects .subject-row').should('have.length', 1);
        cy.get('#field_date_of_birth_to').type('1979-01-01');
        cy.get('#apply-filter').click();
        cy.get('jhi-subjects .subject-row').should('have.length', 0);
        cy.get('#field_date_of_birth_to').clear();
        cy.get('#apply-filter').click();
        cy.get('jhi-subjects .subject-row').should('have.length', 4);
        cy.get('#advanced-filter').click();
    });

    xit('should be able to filter subjects by date of birth from and to', () => {
        cy.get('#advanced-filter').click();
        cy.get('#field_date_of_birth_from').type('1979-01-01');
        cy.get('#field_date_of_birth_to').type('1981-01-01');
        cy.get('#apply-filter').click();
        cy.get('jhi-subjects .subject-row').should('have.length', 1);
        cy.get('#field_date_of_birth_from').type('1981-01-01');
        cy.get('#field_date_of_birth_to').type('1982-01-01');
        cy.get('#apply-filter').click();
        cy.get('jhi-subjects .subject-row').should('have.length', 0);
        cy.get('#field_date_of_birth_from').clear();
        cy.get('#field_date_of_birth_to').clear();
