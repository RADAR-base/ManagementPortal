 ...
 */
@Component
class ManagementPortalOauthKeyStoreHandler
    @Autowired
    constructor(
        ...
        private fun loadStore(): Entry<Resource, KeyStore> {
            for (resource in KEYSTORE_PATHS) {
                ...
                try {
                    val fileName = resource.filename?.lowercase() ?: throw NullPointerException("Resource filename is null")
        ...
        fun loadJwks(): JsonWebKeySet =
            JsonWebKeySet(
                verifierPublicKeyAliasList
                    .map { alias: String -> this.getKeyPair(alias) }
                    .map { keyPair: KeyPair? -> getJwtAlgorithm(keyPair) }
                    .mapNotNull { obj: JwtAlgorithm? -> obj?.jwk },
            )
        ...
        val algorithmForSigning: Algorithm
            ...
            get() {
                ...
                val keyPair =
                    getKeyPair(signKey!!) ?: throw IllegalArgumentException(
         ...
         * loaded.
         */
        private fun getKeyPair(alias: String): KeyPair? = getKeyPair(alias, password)
        ...
        private fun getKeyPair(
            ...
            return try {
                ...
                if (key == null) {
                    logger.warn(
                        "JWT key store {} does not contain private key pair for alias {}",
                        loadedResource,
                        alias,
                    )
                    return null
                ...
                if (cert == null) {
                    logger.warn(
                        "JWT key store {} does not contain certificate pair for alias {}",
                        loadedResource,
                        alias,
                    )
                    return null
                ...
                if (publicKey == null) {
                    logger.warn(
                        "JWT key store {} does not contain public key pair for alias {}",
                        loadedResource,
                        alias,
                    )
                    return null
                ...
                val loaderList =
                    listOf(
                        ...
                        KratosTokenVerifierLoader(
                            managementPortalProperties.identityServer.publicUrl() ?: "",
                            requireAal2 = managementPortalProperties.oauth.requireAal2,
                        ),
                    )
        ...
        companion object {
            ...
            private val KEYSTORE_PATHS =
                listOf<Resource>(
                    ClassPathResource("/config/keystore.p12"),
                    ClassPathResource("/config/keystore.jks"),
                )
            ...
            private fun getAlgorithmFromKeyPair(keyPair: KeyPair): Algorithm {
                val alg =
                    getJwtAlgorithm(keyPair) ?: throw IllegalArgumentException(
            ...
            private fun getJwtAlgorithm(keyPair: KeyPair?): JwtAlgorithm? {
                ...
                return when (val privateKey = keyPair.private) {
                    is ECPrivateKey -> {
                    ...
                    is RSAPrivateKey -> {
                        ...
                        logger.warn(
                            "No JWT algorithm found for key type {}",
                            privateKey.javaClass,
                        )
                        null
                    }
                }
