...
data class EntityDetails(
     ...
     */
    fun minimumEntityOrNull(): Permission.Entity? =
        when {
...
inline fun entityDetails(config: EntityDetails.() -> Unit): EntityDetails = EntityDetails().apply(config)

fun entityDetailsBuilder(config: Consumer<EntityDetails>): EntityDetails = entityDetails(config::accept)
