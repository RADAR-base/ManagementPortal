...
If the app is paired via the Pair App dialog, the QR code that will be scanned contains a token name
and a short-lived URL, e.g.:
```
{
  "tokenName" : "bMUkowOmTOci",
  "tokenUrl" : "https://radar-base-url.org/api/meta-token/bMUkowOmTOci"
}
```
Your app should access the URL specified in the `tokenUrl` field, where it will receive an OAuth2 
