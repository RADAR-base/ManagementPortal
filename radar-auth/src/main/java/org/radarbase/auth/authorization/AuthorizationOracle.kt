...
class AuthorizationOracle(
    ...
    /**
     * Check whether [identity] has permission [permission], regarding given [entity]. The
     * entity can be constructed using a builder pattern. The permission is checked both for its
    ...
    fun checkPermission(
        ...
        entity: Consumer<EntityDetails>
    ) = checkPermission(identity, permission, EntityDetails().apply { entity.accept(this) })

    /**
     * Check whether [identity] has permission [permission], regarding given [entity]. An additional
    ...
    fun checkPermission(
        ...
        if (!hasPermission(identity, permission, entity, entityScope)) {
            throw NotAuthorizedException(
                "User ${identity.username} with client ${identity.clientId} does not have permission $permission to scope " +
                        "$entityScope of $entity"
            )
        }
    }
    ...
    /**
     * Whether [identity] has permission [permission], regarding given [entity].  The
     * entity can be constructed using a builder pattern. The permission is checked both for its
     * own entity scope and for the [EntityDetails.minimumEntityOrNull] entity scope.
     * @return true if identity has permission, false otheriwse
     */
    fun hasPermission(
        ...
        permission: Permission,
        entity: Consumer<EntityDetails>
    ) = hasPermission(identity, permission, EntityDetails().apply { entity.accept(this) })

    /**
     * Check whether given [identity] would have the [permission] scope in any of its roles. This doesn't
    ...
    fun checkScope(
        ...
        if (!hasScope(identity, permission)) {
            throw NotAuthorizedException(
                "User ${identity.username} with client ${identity.clientId} does not have permission $permission"
            )
        }
    }
        ...
        identity.roles.forEach {
            if (it.role.mayBeGranted(permission)) {
                when (it.role.scope) {
                    ...
                    RoleAuthority.Scope.PROJECT -> projects.add(it.referent!!)
                }
            }
        }
    ...
    private fun AuthorityReference.hasAuthority(
        ...
        Permission.Entity.MEASUREMENT -> hasAuthority(identity, permission, entity, Permission.Entity.SOURCE)
