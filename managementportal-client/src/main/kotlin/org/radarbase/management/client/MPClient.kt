...
fun mpClient(config: MPClient.Config.() -> Unit): MPClient = MPClient(MPClient.Config().apply(config))
...
class MPClient(
    ...
    val httpClient =
        (originalHttpClient ?: HttpClient(CIO)).config {
            install(HttpTimeout) {
    ...
    suspend inline fun <reified T> request(crossinline block: HttpRequestBuilder.() -> Unit): T =
    ...
    suspend inline fun requestText(crossinline block: HttpRequestBuilder.() -> Unit): String =
        withContext(Dispatchers.IO) {
            with(httpClient.request(block)) {
                if (!status.isSuccess()) {
                    throw HttpStatusException(status, "Request to ${request.url} failed (code $status)")
